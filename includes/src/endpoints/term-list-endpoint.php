...
class Term_List_Endpoint {
	...
	public function handle() {
		...
		try {
			...
			/**
			 * Filter the query args used to retrieve terms in the term-list endpoint.
			 * Use this to change orderby, order, or any other arg for a specific taxonomy.
			 * @param array  $args          The query args passed to \MyListing\get_terms().
			 * @param string $taxonomy_name The taxonomy name being queried.
			 *
			 * @since 2.15
			 */
			$args = apply_filters( 'mylisting/term-list/query-args', $args, $taxonomy->name );

			$terms = \MyListing\get_terms( $args );

			/**
			 * Filter the terms array returned by the term-list endpoint.
			 * Use this to apply a fully custom order, e.g. a hand-picked slug sequence.
			 *
			 * The array is keyed by term_id (or slug when term-value=slug) with term name as value.
			 *
			 * @param array  $terms         Associative array of [ term_id_or_slug => term_name ].
			 * @param string $taxonomy_name The taxonomy name being queried.
			 *
			 * @since 2.15
			 */
			$terms = apply_filters( 'mylisting/term-list/terms', $terms, $taxonomy->name );
