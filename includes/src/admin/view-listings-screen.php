...
class View_Listings_Screen {
	...
	public function add_filters() {
		...
		$defaults = [];
		foreach ( get_object_taxonomies( 'job_listing', 'objects' ) as $slug => $tax ) {
			$defaults[ $slug ] = $tax->labels->singular_name ?: $tax->name;
		}

		if ( has_filter( 'mylisting\admin\filters\custom_taxonomy_filters' ) ) {

			$filtered = apply_filters( 'mylisting\admin\filters\custom_taxonomy_filters', [] );
			...
			if ( empty( $filtered ) ) {
				...
				foreach ( $filtered as $key => $val ) {
					if ( is_int( $key ) && is_string( $val ) ) {
						$slug = $val;
						$taxonomies[ $slug ] = $defaults[ $slug ] ?? ucwords( str_replace( '_', ' ', $slug ) );
			...
			foreach ( $taxonomies as $tax_slug => $tax_label ) {
				$selected = ! empty( $_GET[ $tax_slug ] )
				? get_term_by( 'slug', $_GET[ $tax_slug ], $tax_slug )
				: false;
				?>
				<select
				class="custom-select"
				name="<?php echo esc_attr( $tax_slug ); ?>"
				id="<?php echo esc_attr( $tax_slug ); ?>"
				...
				placeholder="<?php echo esc_attr( sprintf(
					_x( 'Select %s', 'WP Admin > All Listings Table', 'my-listing' ),
					$tax_label
					) ); ?>"
					>
					<option></option>
					<?php if ( $selected instanceof \WP_Term ) : ?>
						<option
						value="<?php echo esc_attr( $selected->slug ); ?>"
						selected="selected"
						><?php echo esc_html( $selected->name ); ?></option>
					<?php endif; ?>
				</select>
				<?php
			}
