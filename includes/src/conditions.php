...
class Conditions {
	...
	public function get_package_id() {
		...
		// Switch/relist listing form: use the selected package for conditions (field visibility),
		// even if the listing currently has a different package assigned.
		if ( $this->listing && ! empty( $_REQUEST['action'] ) && in_array( $_REQUEST['action'], [ 'switch', 'relist' ], true ) && ! empty( $_REQUEST['listing_package'] ) ) {
