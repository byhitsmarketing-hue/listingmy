...
class Listing {
	...
	private static $distance_request_context = [];
	...
	public function get_marker( $size = 'thumbnail' ) {
		if ( $this->has_field( 'logo' ) && !apply_filters( 'mylisting\listing\use-default-marker', false, $this ) ) {
			return c27()->get_resized_image( $this->get_field( 'logo' ), $size );
		}
	...
	public static function set_distance_request_context( $context ) {
		self::$distance_request_context = self::normalize_distance_request_context( $context );
	}

	public static function get_distance_request_context() {
		return self::$distance_request_context;
	}

	private static function normalize_distance_request_context( $context ) {
		if ( empty( $context ) || ! is_array( $context ) ) {
			return [];
		}

		$search_location = ! empty( $context['search_location'] )
			? sanitize_text_field( wp_unslash( $context['search_location'] ) )
			: '';
		$proximity = isset( $context['proximity'] ) && is_numeric( $context['proximity'] )
			? (float) $context['proximity']
			: 0;
		$lat = isset( $context['lat'] ) && is_numeric( $context['lat'] )
			? (float) $context['lat']
			: false;
		$lng = isset( $context['lng'] ) && is_numeric( $context['lng'] )
			? (float) $context['lng']
			: false;

		if ( $search_location === '' || ! $proximity || $lat === false || $lng === false ) {
			return [];
		}
		...
		$units = 'km';
		if (
			( isset( $context['proximity_units'] ) && $context['proximity_units'] === 'mi' ) ||
			( isset( $context['units'] ) && $context['units'] === 'mi' )
		) {
			$units = 'mi';
		}
	...
	public function get_distance_from_location() {
		...
		if ( is_singular( 'job_listing' ) ) {
			...
			$context = self::get_distance_request_context();

			if ( empty( $context ) && ! empty( $_GET['form_data'] ) && is_array( $_GET['form_data'] ) ) {
				$context = self::normalize_distance_request_context( wp_parse_args( $_GET['form_data'], [
					'proximity_units' => isset( $_GET['proximity_units'] ) ? $_GET['proximity_units'] : null,
				] ) );
			}

			if ( empty( $context ) ) {
				$context = self::normalize_distance_request_context( $_GET );
			}
			...
			$proximity = (float) $context['proximity'];
	...
	public function is_claimable() {
		...
		// if listing is already claimed/verified, it can't be claimed again
		if ( $this->is_verified() ) {
			return false;
		}
