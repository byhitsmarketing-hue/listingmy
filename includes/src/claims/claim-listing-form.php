...
class Claim_Listing_Form extends \MyListing\Src\Forms\Base_Form {
	...
	public function __construct() {
		if ( $this->use_recaptcha_field() ) {
			add_action( 'mylisting/claim/form-fields/end', '\MyListing\display_recaptcha' );
			add_action( 'mylisting/claim/validate-fields', '\MyListing\validate_recaptcha' );
		}
		...
		if ( $this->paid_listings_disabled() ) {
			unset( $this->steps['claim_package'] );
		}
	...
	public function login_register_handler() {
		...
		if ( $this->paid_listings_disabled() && ( isset( $_GET['_claim_id'] ) || ! $this->listing->is_claimable() ) ) {
			$this->next_step();
			return;
		}
	...
	/**
	 * Paid listings disabled
	 *
	 * @return bool
	 */
	public function paid_listings_disabled(): bool {
		$global_enabled   = (bool) mylisting_get_setting( 'paid_listings_enabled' );
		$per_type_enabled = (bool) ( $this->listing->type->settings['packages']['enabled'] ?? false );

		return ! $global_enabled || ! $per_type_enabled;
	}
	...
	public function claim_listing_view() {
		...
		$claim_id = isset( $_GET['_claim_id'] ) ? $_GET['_claim_id'] : false;
		if ( $this->paid_listings_disabled() && empty( $claim_id ) ) {
			do_action( 'mylisting/payments/claim/use-without-package', $this->listing );
			return;
		}
		if ( $this->fields && empty( $claim_id ) ) {
			...
			try {
				...
				if ( ! empty( $_COOKIE['chosen_package_id'] ) ) {
					...
					$package_id = $_REQUEST['listing_package'];
				}
