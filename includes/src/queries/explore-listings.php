	...
	public function run( $request ) {
		...
		if ( $context === 'term-search' ) {
			...
			if ( isset( $form_data['proximity'], $form_data['lat'], $form_data['lng'] ) ) {
				...
				$location = isset( $form_data['search_location'] ) ? sanitize_text_field( wp_unslash( $form_data['search_location'] ) ) : false;
		...
		$distance_context = $form_data;
		if ( isset( $request['proximity_units'] ) && $request['proximity_units'] !== '' ) {
			$distance_context['proximity_units'] = $request['proximity_units'];
		} elseif ( empty( $distance_context['proximity_units'] ) && ( $proximity_filter = $type->get_selected_filter( 'proximity' ) ) ) {
			$distance_context['proximity_units'] = $proximity_filter->get_prop( 'units' ) === 'imperial' ? 'mi' : 'km';
		}

		$previous_distance_context = \MyListing\Src\Listing::get_distance_request_context();
		\MyListing\Src\Listing::set_distance_request_context( $distance_context );
		...
		try {
			...
			\MyListing\Src\Listing::set_distance_request_context( $previous_distance_context );
		}
		...
		// Generate pagination
		$pagination_link = '#';
		if ( ! empty( $request['pagination_link'] ) ) {
			$pagination_link = esc_url_raw( $request['pagination_link'] );
		}

		$result['pagination'] = c27()->get_listing_pagination( $listings->max_num_pages, ( $page + 1 ), $pagination_link );
