...
function normalize_hierarchy_term_assignment( $assign_taxes, $taxonomy, $import_id ) {
	if ( empty( $assign_taxes ) || ! is_array( $assign_taxes ) || ! taxonomy_exists( $taxonomy ) ) {
		return $assign_taxes;
	}

	$type = get_import_listing_type_by_id( $import_id );
	if ( ! $type || ! import_has_hierarchy_field_for_taxonomy( $type, $taxonomy ) ) {
		return $assign_taxes;
	}

	$terms = [];
	foreach ( $assign_taxes as $term_taxonomy_id ) {
		$term = get_term_by( 'term_taxonomy_id', absint( $term_taxonomy_id ), $taxonomy );
		if ( $term instanceof \WP_Term ) {
			$terms[ $term_taxonomy_id ] = $term;
		}
	}
	...
	$leaf_terms = get_selected_leaf_terms( $terms, $taxonomy );
	if ( count( $leaf_terms ) === count( $terms ) ) {
		return $assign_taxes;
	}

	return array_keys( $leaf_terms );
}

function get_import_listing_type_by_id( $import_id ) {
	global $wpdb;
	...
	$options = $wpdb->get_var( $wpdb->prepare( "SELECT options FROM {$wpdb->prefix}pmxi_imports WHERE id = %d", $import_id ) );
	$options = ! empty( $options ) ? maybe_unserialize( $options ) : [];
	$listing_type = ! empty( $options['listing_type'] ) ? $options['listing_type'] : null;

	return \MyListing\Src\Listing_Type::get_by_name( $listing_type );
}

function import_has_hierarchy_field_for_taxonomy( $type, $taxonomy ) {
	foreach ( $type->get_fields() as $field ) {
		if (
			$field->get_type() === 'term-select'
			&& $field->get_prop('terms-template') === 'hierarchy'
...
function get_selected_leaf_terms( $terms, $taxonomy ) {
	...
	foreach ( $terms as $term ) {
		$ancestor_map[ $term->term_id ] = array_map(
			'absint',
			get_ancestors( $term->term_id, $taxonomy, 'taxonomy' )
		);
	}

	return array_filter( $terms, function( $term ) use ( $ancestor_map ) {
		foreach ( $ancestor_map as $term_id => $ancestors ) {
