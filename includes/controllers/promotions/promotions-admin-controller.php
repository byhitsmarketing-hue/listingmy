...
class Promotions_Admin_Controller extends \MyListing\Controllers\Base_Controller {
	...
	protected function hooks() {
		...
		// Native replacement for ACF: Register metabox and save handler for Promotion Package fields
		$this->on( 'add_meta_boxes', '@add_promotion_package_metabox', 100 );
		// Save early so other handlers (e.g. status change logic) can read updated meta.
		$this->once( 'save_post', '@save_promotion_package_metabox', 10, 2 );
	}
	...
	/**
	 * Register native Promotion Package metabox (replaces ACF group "Promotion Package").
	 */
	protected function add_promotion_package_metabox() {
		add_meta_box(
			'ml-promotion-package',
			_x( 'Promotion Package', 'Promotions metabox title', 'my-listing' ),
			[ $this, 'render_promotion_package_metabox' ],
			'cts_promo_package',
			'normal',
			'high'
		);
		...
		remove_meta_box( 'acf-group_5b1a02fc17912', 'cts_promo_package', 'normal' );
	}

	/**
	 * Render native Promotion Package metabox fields.
	 */
	public function render_promotion_package_metabox( $post ) {
		if ( ! ( $post && $post->post_type === 'cts_promo_package' ) ) {
			return;
		}

		// Nonce for saving.
		wp_nonce_field( 'ml_promotion_package_save', 'ml_promotion_package_nonce' );

		// Read existing values.
		$listing_id  = absint( get_post_meta( $post->ID, '_listing_id', true ) );
		$expires_raw = get_post_meta( $post->ID, '_expires', true );
		$expires_dt  = $expires_raw ? date( 'Y-m-d\TH:i', strtotime( $expires_raw ) ) : '';
		...
		// Setup ajax-enabled listing selector to support large sites.
		$listing_select_params = c27()->encode_attr( [
			'post_type'    => 'job_listing',
			'item_key'     => 'ID',
			'post-status'  => [ 'publish', 'pending', 'pending_payment', 'expired', 'draft', 'private' ],
		] );

		$product_select_params = c27()->encode_attr( [
			'product-type' => [ 'promotion_package' ],
			'post-status'  => [ 'publish', 'private' ],
		] );

		$order_select_params = c27()->encode_attr( [] );

		echo '<div class="ml-admin-promo-package">';

		// Promoted Listing (keep class for legacy JS overlay behavior).
		echo '<p class="promote-listing-setting">';
		echo '<label class="ml-backend-label" for="ml_promo_listing_id">' . esc_html__( 'Promoted Listing', 'my-listing' ) . '</label>';
		echo '<select '
			. 'class="custom-select" '
			. 'name="ml_promo_listing_id" '
			. 'id="ml_promo_listing_id" '
			. 'style="width:100%" '
			. 'data-mylisting-ajax="true" '
			. 'data-mylisting-ajax-url="mylisting_list_posts" '
			. 'data-mylisting-ajax-params="' . $listing_select_params . '" '
			. 'placeholder="' . esc_attr__( '- Select listing -', 'my-listing' ) . '"'
			. '>';
		echo '<option></option>';
		if ( $listing_id ) {
			$listing_title = get_the_title( $listing_id );
			if ( $listing_title ) {
				printf( '<option value="%1$s" selected="selected">#%1$s - %2$s</option>', esc_attr( $listing_id ), esc_html( $listing_title ) );
			}
		}
		echo '</select>';
		echo '</p>';

		// Promoted Until (datetime-local)
		echo '<p>';
		echo '<label class="ml-backend-label" for="ml_promo_expires">' . esc_html__( 'Promoted Until', 'my-listing' ) . '</label>';
		echo '<input type="datetime-local" id="ml_promo_expires" name="ml_promo_expires" value="' . esc_attr( $expires_dt ) . '" class="regular-text" />';
		echo '<span class="description" style="display:block;margin-top:4px">' . esc_html__( 'Listing will be promoted until this date, after which, the promotion package expires.', 'my-listing' ) . '</span>';
		echo '</p>';

		// Duration (hidden in UI but kept for compatibility).
		echo '<p class="cts-hidden">';
		echo '<label class="ml-backend-label" for="ml_promo_duration">' . esc_html__( 'Duration (days)', 'my-listing' ) . '</label>';
		echo '<input type="number" min="0" step="1" id="ml_promo_duration" name="ml_promo_duration" value="' . esc_attr( $duration ) . '" placeholder="' . esc_attr__( 'Default', 'my-listing' ) . '" class="small-text" />';
		echo '</p>';

		// Listing Priority
		echo '<p>';
		echo '<label class="ml-backend-label" for="ml_promo_priority">' . esc_html__( 'Listing Priority', 'my-listing' ) . '</label>';
		echo '<input type="number" min="0" step="1" id="ml_promo_priority" name="ml_promo_priority" value="' . esc_attr( $priority ) . '" class="small-text" />';
		echo '<span class="description" style="display:block;margin-top:4px">' . esc_html__( 'Higher value means the listing will have higher priority in search results.', 'my-listing' ) . '</span>';
		echo '</p>';

		// User
		echo '<p>';
		echo '<label class="ml-backend-label" for="ml_promo_user_id">' . esc_html__( 'User', 'my-listing' ) . '</label>';
		echo '<select '
			. 'class="custom-select" '
			. 'name="ml_promo_user_id" '
			. 'id="ml_promo_user_id" '
			. 'style="width:100%" '
			. 'data-mylisting-ajax="true" '
			. 'data-mylisting-ajax-url="mylisting_list_users" '
			. 'placeholder="' . esc_attr__( '- None -', 'my-listing' ) . '"'
			. '>';
		echo '<option></option>';
		if ( $user_id && ( $user = get_userdata( $user_id ) ) ) {
			printf( '<option value="%1$s" selected="selected">%2$s</option>', esc_attr( $user_id ), esc_html( $user->display_name ) );
		...
		// Product
		echo '<p>';
		echo '<label class="ml-backend-label" for="ml_promo_product_id">' . esc_html__( 'Product', 'my-listing' ) . '</label>';
		echo '<select '
			. 'class="custom-select" '
			. 'name="ml_promo_product_id" '
			. 'id="ml_promo_product_id" '
			. 'style="width:100%" '
			. 'data-mylisting-ajax="true" '
			. 'data-mylisting-ajax-url="mylisting_list_products" '
			. 'data-mylisting-ajax-params="' . $product_select_params . '" '
			. 'placeholder="' . esc_attr__( '- None -', 'my-listing' ) . '"'
			. '>';
		echo '<option></option>';
		if ( $product_id && ( $product = wc_get_product( $product_id ) ) ) {
			printf( '<option value="%1$s" selected="selected">#%1$s - %2$s</option>', esc_attr( $product_id ), esc_html( $product->get_name() ) );
		}
		echo '</select>';
		echo '</p>';
		...
		// Order
		echo '<p>';
		echo '<label class="ml-backend-label" for="ml_promo_order_id">' . esc_html__( 'Order', 'my-listing' ) . '</label>';
		echo '<select '
			. 'class="custom-select" '
			. 'name="ml_promo_order_id" '
			. 'id="ml_promo_order_id" '
			. 'style="width:100%" '
			. 'data-mylisting-ajax="true" '
			. 'data-mylisting-ajax-url="mylisting_list_orders" '
			. 'data-mylisting-ajax-params="' . $order_select_params . '" '
			. 'placeholder="' . esc_attr__( '- None -', 'my-listing' ) . '"'
			. '>';
		echo '<option></option>';
		if ( $order_id && function_exists( 'wc_get_order' ) && ( $order = wc_get_order( $order_id ) ) ) {
			$label = sprintf( '#%s - %s', $order->get_order_number(), $order->get_date_created() ? $order->get_date_created()->date_i18n( 'F j, Y g:i a' ) : '' );
			printf( '<option value="%1$s" selected="selected">%2$s</option>', esc_attr( $order_id ), esc_html( $label ) );
		}
		echo '</select>';
	...
	/**
	 * Save native Promotion Package metabox.
	 */
	protected function save_promotion_package_metabox( $post_id, $post ) {
		if ( ! ( $post && $post->post_type === 'cts_promo_package' ) ) {
			return;
		}

		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Reading nonce from $_POST to verify it.
		if ( empty( $_POST['ml_promotion_package_nonce'] ) || ! wp_verify_nonce( $_POST['ml_promotion_package_nonce'], 'ml_promotion_package_save' ) ) {
			return;
		}
		...
		$listing_id  = isset( $_POST['ml_promo_listing_id'] ) ? absint( $_POST['ml_promo_listing_id'] ) : 0;
		$expires_in  = isset( $_POST['ml_promo_expires'] ) ? sanitize_text_field( wp_unslash( $_POST['ml_promo_expires'] ) ) : '';
		$duration    = isset( $_POST['ml_promo_duration'] ) ? absint( $_POST['ml_promo_duration'] ) : '';
		$priority    = isset( $_POST['ml_promo_priority'] ) ? absint( $_POST['ml_promo_priority'] ) : '';
		$user_id     = isset( $_POST['ml_promo_user_id'] ) ? absint( $_POST['ml_promo_user_id'] ) : 0;
		$product_id  = isset( $_POST['ml_promo_product_id'] ) ? absint( $_POST['ml_promo_product_id'] ) : 0;
		$order_id    = isset( $_POST['ml_promo_order_id'] ) ? absint( $_POST['ml_promo_order_id'] ) : 0;
		...
		$expires = '';
		if ( ! empty( $expires_in ) ) {
			$ts = strtotime( $expires_in );
			if ( $ts ) {
				$expires = date( 'Y-m-d H:i:s', $ts );
			}
		}

		// Persist meta (add/update or delete when empty).
		$listing_id ? update_post_meta( $post_id, '_listing_id', $listing_id ) : delete_post_meta( $post_id, '_listing_id' );
		$expires    ? update_post_meta( $post_id, '_expires', $expires )       : delete_post_meta( $post_id, '_expires' );
		( $duration !== '' ) ? update_post_meta( $post_id, '_duration', $duration ) : delete_post_meta( $post_id, '_duration' );
		( $priority !== '' ) ? update_post_meta( $post_id, '_priority', $priority ) : delete_post_meta( $post_id, '_priority' );
