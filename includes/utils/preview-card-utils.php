...
/**
 * Sanitize listing type editor strings used in preview cards.
 *
 * These strings are rendered inside the Explore Vue mount element during SSR
 * (server-side first load), so malformed HTML or Vue template delimiters can
 * break the Explore page before any JS runs.
 *
 * @since 2.14
 */
function sanitize_preview_card_string( $string, $listing = null ) {
	if ( ! is_string( $string ) || $string === '' ) {
		return '';
	}

	if ( apply_filters( 'mylisting/preview-card/sanitize-editor-html', false, $string, $listing ) !== true ) {
		return $string;
	}

	// Prevent user-provided strings from being treated as Vue template expressions.
	$string = str_replace( [ '{{', '}}' ], [ '&#123;&#123;', '&#125;&#125;' ], $string );

	// Attempt to fix unbalanced tags, then strip/normalize unsupported markup/attributes.
	$string = force_balance_tags( $string );

	$allowed = (array) apply_filters( 'mylisting/preview-card/allowed-string-html', [
		'a' => [
			'href' => true,
			'title' => true,
			'target' => true,
		...
		if ( $var === 'upcoming' ) {
			foreach ( $value as $field_key => $upcoming ) {
				...
				if ( ! empty( $dates ) ) {
					...
					$start_raw = \MyListing\Src\Recurring_Dates\display_instance( $dates[0], 'start', $reference );
					$end_raw = \MyListing\Src\Recurring_Dates\display_instance( $dates[0], 'end', $reference );
					$datetime_fmt = sprintf( '%s %s', get_option( 'date_format' ), get_option( 'time_format' ) );
					$start = $start_raw ? date_i18n( $datetime_fmt, strtotime( $start_raw ) ) : '';
					$end = $end_raw ? date_i18n( $datetime_fmt, strtotime( $end_raw ) ) : '';
				} else {
					$visible = 'style="display:none;"';
