...
class Elementor {
	...
	public function __construct() {
		...
		add_action( 'wp_enqueue_scripts', [ $this, 'enqueue_fontawesome_styles' ], 11 );
		add_action( 'wp_enqueue_scripts', [ $this, 'enqueue_theme_builder_location_styles' ], 19 );
		add_action( 'elementor/frontend/before_enqueue_styles', [ $this, 'register_theme_builder_location_style_ids' ] );
		...
		if ( is_admin() ) {
			add_filter( 'plugin_action_links_elementor/elementor.php', function( $links ) {
				$affiliate_id = '230281';

				if ( empty( $links['go_pro'] ) || strpos( $links['go_pro'], 'elementor-plugins-gopro' ) === false ) {
					return $links;
				}

				$links['go_pro'] = str_replace(
					'https://go.elementor.com/go-pro-wp-plugins/',
					sprintf( 'https://go.elementor.com/visit/?bta=%s&brand=elementor', $affiliate_id ),
					$links['go_pro']
				);
	...
	public function enqueue_theme_builder_location_styles() {
		if ( ! class_exists( '\Elementor\Plugin' ) ) {
			return;
		}
		...
		if ( empty( $this->get_theme_builder_location_template_ids() ) ) {
			return;
		}
	...
	public function enqueue_fontawesome_styles() {
		if ( ! class_exists( '\Elementor\Plugin' ) || ! class_exists( '\Elementor\Icons_Manager' ) ) {
			return;
		}
	...
	public function register_theme_builder_location_style_ids() {
		if ( ! class_exists( '\Elementor\Plugin' ) ) {
			return;
		}
		...
		// Elementor's atomic styles manager collects post IDs from elementor/post/render
		// before it enqueues them on elementor/frontend/after_enqueue_post_styles.
		foreach ( $this->get_theme_builder_location_template_ids() as $template_id ) {
			do_action( 'elementor/post/render', $template_id );
		}
	}

	private function get_theme_builder_location_template_ids() {
		static $template_ids = null;
					...
					foreach ( [ 'header', 'footer' ] as $location ) {
						...
						foreach ( $documents as $document_id => $document ) {
							if ( is_object( $document ) && method_exists( $document, 'get_main_id' ) ) {
								$template_ids[] = $document->get_main_id();
							} elseif ( is_object( $document ) && method_exists( $document, 'get_id' ) ) {
								$template_ids[] = $document->get_id();
							} elseif ( is_numeric( $document_id ) ) {
								$template_ids[] = $document_id;
							}
						}
					}
				}
		...
		if ( function_exists( 'get_hfe_header_id' ) ) {
			$template_ids[] = get_hfe_header_id();
		}

		if ( function_exists( 'get_hfe_footer_id' ) ) {
			$template_ids[] = get_hfe_footer_id();
		}

		return array_unique( array_filter( array_map( 'absint', $template_ids ) ) );
	}
